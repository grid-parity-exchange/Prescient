...
class MutableSimulationState(SimulationState):
    ...
    def apply_actuals(self, options, actuals) -> None:
        ...
        _save_forecastables(options, actuals, self._actuals, max_length_multiplier=int(60//self._sced_frequency))
...
def _save_forecastables(options, ruc, where_to_store, max_length_multiplier=1):
    first_ruc = (len(where_to_store) == 0)
    ruc_delay = -(options.ruc_execution_hour % (-options.ruc_every_hours))
    max_length = max_length_multiplier*(ruc_delay + options.ruc_horizon)
    ...
    for idx, (new_ruc_vals,) in enumerate(get_forecastables(ruc)):
        if first_ruc:
            ...
            for _ in range(len(forecast) - max_length_multiplier*ruc_delay):
                forecast.pop()
