...
def parse_args(args: List[str]) -> ConfigDict:
    ...
    simulator_config.import_argparse(args)
...
def _construct_options_parser() -> ArgumentParser:
    ...
    def outer_parse(args=None, values=None):
        ...
        simulator_config.initialize_argparse(parser)
...
simulator_config.declare("plugin_modules", ConfigList(
    domain=str,
    default=[],
    description="The path of a python module that extends prescient behavior",
)).declare_as_argument("--plugin")
...
simulator_config.declare("profile", ConfigValue(
    domain=NonNegativeInt,
    default=0,
    description="Enable profiling of Python code. "
                "The value of this option is the number of functions that are summarized.",
)).declare_as_argument()

simulator_config.declare("traceback", ConfigValue(
    domain=bool,
    default=False,
    description="When an exception is thrown, show the entire call stack. "
