...
# defaults from a script.
need_mutable_parameters = True
...
model.ReserveRequirement = Param(model.TimePeriods, within=NonNegativeReals, default=0.0, mutable=need_mutable_parameters)
...
def scale_ramp_up(m, g):
    ...
    if temp > m.MaximumPowerOutput[g]:
        ...
        return temp
model.ScaledNominalRampUpLimit = Param(model.ThermalGenerators, within=NonNegativeReals, initialize=scale_ramp_up, mutable=need_mutable_parameters)
...
def scale_ramp_down(m, g):
    ...
    if temp > m.MaximumPowerOutput[g]:
        ...
        return temp
model.ScaledNominalRampDownLimit = Param(model.ThermalGenerators, within=NonNegativeReals, initialize=scale_ramp_down, mutable=need_mutable_parameters)
...
def scale_startup_limit(m, g):
    ...
    if temp > m.MaximumPowerOutput[g]:
        ...
        return temp
model.ScaledStartupRampLimit = Param(model.ThermalGenerators, within=NonNegativeReals, validate=ramp_limit_validator, initialize=scale_startup_limit, mutable=need_mutable_parameters)
...
def scale_shutdown_limit(m, g):
    ...
    if temp > m.MaximumPowerOutput[g]:
        ...
        return temp
model.ScaledShutdownRampLimit = Param(model.ThermalGenerators, within=NonNegativeReals, validate=ramp_limit_validator, initialize=scale_shutdown_limit, mutable=need_mutable_parameters)
...
# verify that all quick start generators have minimum up/down times of 1
def verify_quick_start_minimum_up_and_down_times(m, g):
    if m.MinimumUpTime[g] != 1 or m.MinimumDownTime[g] != 1:
        print("DATA ERROR: Quick start generators must have minimum up and down times of 1. Minimum up and down times for generator=%s are respectively %d and %d" % (g, value(m.MinimumUpTime[g]), value(m.MinimumDownTime[g])))        
        return False
    else:
        return True
model.VerifyQuickStartMinimumUpAndDownTimes = BuildCheck(model.QuickStartGenerators, rule=verify_quick_start_minimum_up_and_down_times)
...
model.NumGeneratorCostCurvePieces = Param(within=PositiveIntegers, default=2, mutable=need_mutable_parameters)
...
# voltage angles at the buses (S) (lock the first bus at 0) in degrees
model.Angle = Var(model.Buses, model.TimePeriods, within=Reals, bounds=(-180.0, 180.0))
...
def power_limit_from_start_stop_rule(m,g,t):
    include_UT_1 = False
    if (not include_UT_1) and (value(m.MinimumUpTime[g]) == 1):
        return Constraint.Skip
    ## time to ramp-up
    time_RU = math.floor( value(m.MaximumPowerOutput[g] - m.ScaledStartupRampLimit[g])/value(m.ScaledNominalRampUpLimit[g]) )
    if t == m.NumTimePeriods: 
        return m.MaximumPowerAvailable[g,t] <= (m.MaximumPowerOutput[g]) *m.UnitOn[g,t] \
...
def power_limit_from_start_stops_rule(m,g,t):
    time_RU = math.floor( value(m.MaximumPowerOutput[g] - m.ScaledStartupRampLimit[g])/value(m.ScaledNominalRampUpLimit[g]) )
    time_RD = math.floor( value(m.MaximumPowerOutput[g] - m.ScaledShutdownRampLimit[g])/value(m.ScaledNominalRampDownLimit[g]) )
...
if storage_services:
    ...
    def energy_conservation_rule(m, s, t):
        # storage s, time t
        if t == m.InitialTime:
            return m.SocStorage[s, t] == m.StorageSocOnT0[s]*m.RetentionRate[s]  + \
...
model.ComputeStartupCosts=Constraint(model.ThermalGenerators, model.TimePeriods, rule=ComputeStartupCost_rule)
...
def fix_binary_variables(instance):
    ...
    for t in instance.TimePeriods:
        ...
        for g in instance.ThermalGenerators:
            instance.UnitOn[g,t].fix()
            instance.UnitStart[g,t].fix()
            instance.UnitStop[g,t].fix()
            
            if regulation_services:
                instance.RegulationOn[g,t].fix()
                
        if storage_services:
            for s in instance.Storage:
                instance.OutputStorage[s,t].fix()

    for g in instance.ThermalGenerators:
        for t_prime, t in instance.ShutdownHotStartupPairs[g]:
            instance.StartupIndicator[g,t_prime,t].fix()
...
def free_binary_variables(instance):
    ...
    for t in instance.TimePeriods:
        ...
        for g in instance.ThermalGenerators:
            instance.UnitOn[g,t].unfix()
            instance.UnitStart[g,t].unfix()
            instance.UnitStop[g,t].unfix()
            
            if regulation_services:
                instance.RegulationOn[g,t].unfix()
                
        if storage_services:
            for s in instance.Storage:
                instance.OutputStorage[s,t].unfix()

    for g in instance.ThermalGenerators:
        for t_prime, t in instance.ShutdownHotStartupPairs[g]:
            instance.StartupIndicator[g,t_prime,t].unfix()
...
def reconstruct_instance_for_t0_changes(instance):
    ...
    del instance.InitialTimePeriodsOnLine
    del instance.InitialTimePeriodsOffLine

    del instance.EnforceMaxAvailableRampUpRates
    del instance.ComputeStartupCosts
    del instance.ComputeShutdownCosts
    del instance.EnforceUpTimeConstraintsInitial
    del instance.EnforceDownTimeConstraintsInitial
    del instance.start_stop

    del instance.PopulateReserveRequirements

    del instance.VerifyMustRunT0StateConsistency

    # IMPT: Due to the comment above, the definitions below should be identical to those in the main model definition.

    instance.InitialTimePeriodsOnLine = Param(instance.ThermalGenerators, within=NonNegativeIntegers, initialize=initial_time_periods_online_rule, mutable=need_mutable_parameters)
    instance.InitialTimePeriodsOffLine = Param(instance.ThermalGenerators, within=NonNegativeIntegers, initialize=initial_time_periods_offline_rule, mutable=need_mutable_parameters)

    instance.EnforceMaxAvailableRampUpRates_New = Constraint(instance.ThermalGenerators, instance.TimePeriods, rule=enforce_max_available_ramp_up_rates_rule)
    instance.EnforceScaledNominalRampDownLimits_New = Constraint(instance.ThermalGenerators, instance.TimePeriods, rule=enforce_ramp_down_limits_rule)
    instance.ComputeStartupCosts_New = Constraint(instance.ThermalGenerators, instance.TimePeriods, rule=ComputeStartupCost_rule)
    instance.ComputeShutdownCosts_New = Constraint(instance.ThermalGenerators, instance.TimePeriods, rule=compute_shutdown_costs_rule)
    instance.EnforceUpTimeConstraintsInitial_New = Constraint(instance.ThermalGenerators, rule=enforce_up_time_constraints_initial)
    instance.EnforceDownTimeConstraintsInitial_New = Constraint(instance.ThermalGenerators, rule=enforce_down_time_constraints_initial)
    instance.start_stop_New=Constraint(instance.ThermalGenerators, instance.TimePeriods,rule=start_stop_rule)

    instance.PopulateReserveRequirements_New = BuildAction(rule=populate_reserve_requirements_rule)

    instance.VerifyMustRunT0StateConsistency_New = BuildAction(instance.ThermalGenerators, rule=verify_must_run_t0_state_consistency_rule)
