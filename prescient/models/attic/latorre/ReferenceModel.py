...
# latorre from Carrion and Arroyo v2 (so no multi-stage) #
########################################################################################################
...
model.BusZone = Param(model.Buses, mutable=need_mutable_parameters)
...
def compute_last_time_period_rule(m):
    return m.InitialTime + m.NumTimePeriods - 1
model.LastTimePeriod = Param(initialize=compute_last_time_period_rule)
...
def between_limits_validator(m, v, g):
   ...
   unit_on_t0 = value(m.UnitOnT0[g])
   status = (v <= max_output * unit_on_t0) and (v >= (min_output * unit_on_t0))
   if status == False:
      print("Failed to validate PowerGeneratedT0 value for g="+g+"; new value="+str(v)+", UnitOnT0="+str(unit_on_to))
   return status 
...
def constrain_unit_start_domain(m):
    for g in m.ThermalGenerators:
        for t in range(m.MinimumUpTime[g],m.LastTimePeriod+1):
            yield (g,t)

model.ConstrainUnitStartDomain = Set(within=model.ThermalGenerators*model.TimePeriods, initialize=constrain_unit_start_domain)
...
model.ConstrainUnitStart = Constraint(model.ConstrainUnitStartDomain, rule=unit_start_rule)

model.UnitStop = Var(model.ThermalGenerators, model.TimePeriods, bounds=(0,1))

def constrain_unit_stop_domain(m):
    for g in m.ThermalGenerators:
        for t in range(m.MinimumDownTime[g],m.LastTimePeriod+1):
            yield (g,t)

model.ConstrainUnitStopDomain = Set(within=model.ThermalGenerators*model.TimePeriods, initialize=constrain_unit_stop_domain)
...
model.ConstraintUnitStop = Constraint(model.ConstrainUnitStopDomain, rule=unit_stop_rule)
...
def delta_domain_g_s_initialization_rule(m):
    for g in m.ThermalGenerators:
        num_startup_segments = len(m.StartupCosts[g])
        for s in m.TimePeriods:
            if s <= num_startup_segments:
                yield (g,s)

model.delta_domain_g_s = Set(dimen=2, within=model.ThermalGenerators*model.TimePeriods, initialize=delta_domain_g_s_initialization_rule)

model.delta=Var(model.delta_domain_g_s * model.TimePeriods, bounds=(0,1))
...
def delta_ineq_initialization_rule(m):
    for g in m.ThermalGenerators:
        num_startup_indices = len(m.StartupCostIndices[g])
        for s in m.TimePeriods:
            for t in m.TimePeriods:
                if s < num_startup_indices and t >= m.StartupLags[g][s+1]:                 
                    yield (g,s,t)

model.delta_ineq_domain = Set(dimen=3, initialize=delta_ineq_initialization_rule)

def delta_ineq_rule(m,g,s,t):
    return sum(m.UnitStop[g,t-i] for i in m.TimePeriods \
               if i >= m.StartupLags[g][s] and i <= m.StartupLags[g][s+1]-1) >= \
           m.delta[g,s,t]

model.delta_ineq=Constraint(model.delta_ineq_domain, rule=delta_ineq_rule)
