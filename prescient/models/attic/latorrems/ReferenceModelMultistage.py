...
# latorrev2  from Carrion and Arroyo v2 #
#this is a model with multi-stage, created by Paulin J. in may 2015 from latorre model
########################################################################################################
...
#This is done automatically when an option file exists in the simulator directory
# similarly, if we use two stage models in a multi stage simulations and want to use commitment costs, the variable useLastCommitments is set to True

print('Loading the multistage latorre model')

'''try :
    with open( os.pardir+ os.sep+"simulator" + os.sep + "MSSim_model_options.tmp",'r' ) as MSSim_option_file :
        line=MSSim_option_file.readline().rstrip('\n')        
        print("found MSSim option file containing : " + line)
        if line=="multistage":
            multistage=True
            useLastCommitments=False
        elif line=="useLastCommitments":
            useLastCommitments=True
            multistage=False
        else:
            print("read " + line +". MSSim option file should contain either 'multistage' or 'useLastCommitments'.")
        
except : # we don't have options, so multistage and useLastCOmmitments are defaulted to False
    print("No MSSim options file found, 'multistage' and 'useLastCommitments' are defaulted to False.")
    multistage=False
    useLastCommitments=False'''
...
model.name = "Latorrefromcarrionarroyov2"
...
#this defines the set from 1 to 48
       ...
       if value(m.ProductionCostA2[g]) == 0:
          ...
          if min_power != max_power:
           ...
           if width == 0:
               ...
               m.PowerGenerationPiecewisePoints[g, t].extend([min_power + i*width for i in range(0,n+1)])
...
if multistage:
    ...
    # this sets up default values but has to be changed with data

    def compute_commitment_fixed_cost_rule(m, g, s):
        return m.MinimumProductionCost[g]
    model.CommitmentFixedCost = Param(model.ThermalGenerators, model.StageSet, within=NonNegativeReals, initialize=compute_commitment_fixed_cost_rule,mutable=True)
    ...
    # adding stage parameter for this cost by Paulin, april 2015
    def compute_decommitment_fixed_cost_rule(m, g,s): 
        return m.MinimumProductionCost[g]
    model.deCommitmentFixedCost = Param(model.ThermalGenerators,model.StageSet, within=NonNegativeReals, initialize=compute_decommitment_fixed_cost_rule,mutable=True) # units are $.
    ...
    def compute_decommitment_costs_rule(m, g, s, t):
        i = m.StageIndex[s]
        if s == m.StageSet[1]:
            return m.deCommitmentCost[g, s, t] >= m.deCommitmentFixedCost[g,s] * (m.UnitCommitedT0[g, t] - m.UnitCommited[g, s, t])
        else:
            return m.deCommitmentCost[g, s, t] >= m.deCommitmentFixedCost[g,s] * (m.UnitCommited[g, m.StageSet[i-1], t] - m.UnitCommited[g, s, t])
    ...
    def relate_planning_and_physical_variables(m, g, t):
        for s in m.StageSet:
            if t in m.GenerationTimeInStage[s]:# we take the generation stage because the last commitment prior to t is made in the immediately prior RUC to this stage
                break
...
if useLastCommitments:
    model.lastCommitments=Param(model.ThermalGenerators,model.TimePeriods,within=Binary,initialize=0,mutable=True)
    model.FixedCommitmentCostInThatStage=Param(model.ThermalGenerators,within=NonNegativeReals,initialize=0.0,mutable=True)
    model.FixedDecommitmentCostInThatStage=Param(model.ThermalGenerators,within=Reals,initialize=0.0,mutable=True)
    model.CommitmentCostInThatStage=Var(model.ThermalGenerators,model.TimePeriods,within=Reals,initialize=0.0)
    model.DecommitmentCostInThatStage=Var(model.ThermalGenerators,model.TimePeriods,within=Reals,initialize=0.0)

    # Cost of commiting units in that intra day stage
    def compute_CommitmentCostInThatStage_rule(m, g,t):
        return m.CommitmentCostInThatStage[g,t] >= m.FixedCommitmentCostInThatStage[g] * (m.UnitOn[g,t] - m.lastCommitments[g, t])
    model.ComputeCommitmentCostInThatStage = Constraint(model.ThermalGenerators,model.TimePeriods,rule=compute_CommitmentCostInThatStage_rule)
    
    def compute_DecommitmentCostInThatStage_rule(m, g,t):
        return m.DecommitmentCostInThatStage[g,t] >= m.FixedDecommitmentCostInThatStage[g] * ( m.lastCommitments[g, t]- m.UnitOn[g,t])
    model.ComputeDecommitmentCostInThatStage = Constraint(model.ThermalGenerators,model.TimePeriods,rule=compute_DecommitmentCostInThatStage_rule)
...
def commitment_in_stage_st_cost_rule(m, st):
    if not multistage: # if we are not solving the multistage version the startup and shutdown costs are first stage costs.
        ...
        if useLastCommitments:
            cc+=(sum(m.CommitmentCostInThatStage[g,t]+m.DecommitmentCostInThatStage[g,t] for g in m.ThermalGenerators for t in m.CommitmentTimeInStage[st]))
    else:
...
# IMPT: Don't include posReserveMismatch for cost penalty, because it just means there far more reserve than necessary in the system.
def generation_in_stage_st_cost_rule(m, st):
    ...
    if multistage: # if we are solving multistage Startup and Shutdown are physical changes, so they are considered as part of the generation costs
...
###### everything above is the carrion arroyo v2 model - elements appearing below ######
###### are additions to the base model, including 3-binary and strengthening.     ######

# begin 3-binary

model.UnitStart=Var(model.ThermalGenerators,model.TimePeriods, bounds=(0,1))

def unit_start_rule(m,g,t):
    if t < m.MinimumUpTime[g]:
        return Constraint.Skip
    else: 
        return sum(m.UnitStart[g,i] for i in m.TimePeriods if i >= t - m.MinimumUpTime[g] + 1 and i <= t) <= m.UnitOn[g,t] 

model.unit_start=Constraint(model.ThermalGenerators, model.TimePeriods, rule=unit_start_rule)

model.UnitStop=Var(model.ThermalGenerators,model.TimePeriods,bounds=(0,1))

def unit_stop_rule(m,g,t):
    if t < m.MinimumDownTime[g]:
        return Constraint.Skip
    else: 
        return sum(m.UnitStop[g,i] for i in m.TimePeriods if i >= t - m.MinimumDownTime[g] + 1 and i <= t) <= 1 - m.UnitOn[g,t] 
...
def power_limit_from_start_rule(m,g,t):
    if m.MinimumUpTime[g] != 1:
        return Constraint.Skip
    return m.MaximumPowerAvailable[g,t] <= m.MaximumPowerOutput[g] * m.UnitOn[g,t] - (m.MaximumPowerOutput[g] - m.StartupRampLimit[g])*m.UnitStart[g,t]
...
def power_limit_from_stop_rule(m,g,t):
    if m.MinimumUpTime[g] != 1 or t == m.NumTimePeriods:
        return Constraint.Skip
    return m.MaximumPowerAvailable[g,t] <= m.MaximumPowerOutput[g]* m.UnitOn[g,t] - (m.MaximumPowerOutput[g] - m.ShutdownRampLimit[g])*m.UnitStop[g,t+1]
...
def power_limit_from_start_stop_rule(m,g,t):
    if m.MinimumUpTime[g] == 1 or t == m.NumTimePeriods:
        return Constraint.Skip
    return m.MaximumPowerAvailable[g,t] <= m.MaximumPowerOutput[g]*m.UnitOn[g,t] - (m.MaximumPowerOutput[g] - m.StartupRampLimit[g])*m.UnitStart[g,t] \
- (m.MaximumPowerOutput[g] - m.ShutdownRampLimit[g])*m.UnitStop[g,t+1]
...
#begin latorre
def delta_domain_rule(m,g,s,t):
    if s > len(m.StartupCosts[g]):
        return False
    return True

model.delta_domain=Set(dimen=3,\
initialize=model.ThermalGenerators * model.TimePeriods * \
model.TimePeriods,filter=delta_domain_rule)

model.delta=Var(model.delta_domain,bounds=(0,1))
...
def delta_ineq_rule(m,g,s,t):
    if s >= len(m.StartupCostIndices[g]) or t<list(m.StartupLags[g])[s]:
        return Constraint.Skip
    return sum(m.UnitStop[g,t-i] for i in m.TimePeriods if \
i >= list(m.StartupLags[g])[s-1] and i <= list(m.StartupLags[g])[s]-1) >= \
m.delta[g,s,t]

model.delta_ineq=Constraint(model.ThermalGenerators,model.TimePeriods,model.TimePeriods,rule=delta_ineq_rule)
