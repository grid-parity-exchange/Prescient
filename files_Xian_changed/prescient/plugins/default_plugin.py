...
from prescient.scripts.simulator import call_solver, DEFAULT_MAX_LABEL_LENGTH, round_small_values
from prescient.plugins.uc_data_generator import _default_get_data_dict as _get_data_dict
...
def create_sced_instance(sced_model, reference_model_module,
    ...
    if use_prescient_forecast_error:
        ...
        if today_stochastic_scenario_instances == None:  # we're running in deterministic mode
            ...
            #         print('Time: {} Bus: {};\nActual Demand: {};\nForecasted Demand: {}'.format(t,b,\
            #         actual_demand[b, hour_to_simulate+t],demand_dict[(b, t+1)]))
    ...
    # pass more info to this function, e.g. data
    ed_data = _get_data_dict( ruc_instance_to_simulate, sced_horizon, demand_dict, None, options.reserve_factor,\
                              ...
                              day_to_simulate,hour_to_simulate-1,bidding = options.bidding,optimization=True)
...
def get_dispatch_levels(deterministic_instance,generator,verbose = False):
    '''
    Input:
    1. deterministic_instance: this instance can be RUC or SCED
    2. generator: the generator we are interested in (str)
    Output:
    1. dispatch_levels: a dict: key generator name, value np array of dispatch levels
    '''

    g = generator
    dispatch_levels = {g:[value(deterministic_instance.PowerGenerated[g,t]) \
    for t in range(1,value(deterministic_instance.NumTimePeriods)+1)]}

    #print('Debug: the length of the list {}'.format(len(dispatch_levels[g])))

    if verbose:
        print(g)
        for t in range(1, value(deterministic_instance.NumTimePeriods)+1):
            print("%7.2f"% value(deterministic_instance.PowerGenerated[g,t]), end=' ')
        print("")
...
def _create_deterministic_ruc(options,
    ...
    ruc_data = _get_ruc_data(options, today_data, this_hour, next_day_data,this_date, UnitOnT0Dict, UnitOnT0StateDict, PowerGeneratedT0Dict, projected_sced, ruc_horizon, use_next_day_in_ruc, today_actuals, next_day_actuals, optimization=True)
...
def _get_ruc_data(options,
                  today_data, this_hour, next_day_data, day_to_simulate,
                  UnitOnT0Dict=None, UnitOnT0StateDict=None, PowerGeneratedT0Dict=None,
                  projected_sced = None,
                  ruc_horizon = 48,
                  use_next_day_in_ruc=False,
                  today_actuals=None, next_day_actuals=None, optimization=False):
    ...
    ruc_data = _get_data_dict(today_data, ruc_horizon, demand_dict, reserve_dict, options.reserve_factor, \
                              min_nondispatch_dict, max_nondispatch_dict, \
                              UnitOnT0Dict, UnitOnT0StateDict, PowerGeneratedT0Dict, StorageSocOnT0Dict,
                              day_to_simulate, None, bidding = options.bidding, optimization=optimization)
...
def create_ruc_instance_to_simulate_next_period(ruc_model, options, this_date, this_hour, next_date):
    ...
    ruc_data = _get_ruc_data(options, ruc_instance_to_simulate_this_period, this_hour, ruc_instance_to_simulate_next_period, this_date,optimization=False )
