...
# # PRESCIENT_INPUT_OPTIONS
    ...
    ################ Xian: Add options for tracking model ######################
    input_simulation_options.add_option('--track-ruc-signal',
                                        help='When tracking the market signal, RUC signals are used instead of the SCED signal.',
                                        action='store_true',
                                        dest='track_ruc_signal',
                                        default=False)
    guiOverride['--track-ruc-signal'] = {}
    guiOverride['--track-ruc-signal']['bpa'] = False

    input_simulation_options.add_option('--track-sced-signal',
                                        help='When tracking the market signal, SCED signals are used instead of the RUC signal.',
                                        action='store_true',
                                        dest='track_sced_signal',
                                        default=False)
    guiOverride['--track-sced-signal'] = {}
    guiOverride['--track-sced-signal']['bpa'] = False

    input_simulation_options.add_option('--hybrid-tracking',
                                        help='When tracking the market signal, hybrid model is used.',
                                        action='store_true',
                                        dest='hybrid_tracking',
                                        default=False)
    guiOverride['--hybrid-tracking'] = {}
    guiOverride['--hybrid-tracking']['bpa'] = False

    input_simulation_options.add_option('--track-horizon',
                                        ...
                                        type='int',
                                        default=48)
    guiOverride['--track-horizon'] = {}
    guiOverride['--track-horizon']['bpa'] = False

    input_simulation_options.add_option('--bidding-generator',
                                        help="Specifies the generator we derive bidding strategis for.",
                                        action='store',
                                        dest='bidding_generator',
                                        type='string',
                                        default='102_STEAM_3')
    guiOverride['--bidding-generator'] = {}
    guiOverride['--bidding-generator']['bpa'] = False

    input_simulation_options.add_option('--bidding',
                                        help="Invoke generator strategic bidding when simulate.",
                                        action='store_true',
                                        dest='bidding',
                                        default=False)
    guiOverride['--bidding'] = {}
    guiOverride['--bidding']['bpa'] = False

    input_simulation_options.add_option('--deviation-weight',
                                        help="Set the weight for deviation term when tracking",
                                        action='store',
                                        dest='deviation_weight',
                                        type='float',
                                        default=30)
    guiOverride['--deviation-weight'] = {}
    guiOverride['--deviation-weight']['bpa'] = False

    input_simulation_options.add_option('--ramping-weight',
                                        help="Set the weight for ramping term when tracking",
                                        action='store',
                                        dest='ramping_weight',
                                        type='float',
                                        default=20)
    guiOverride['--ramping-weight'] = {}
    guiOverride['--ramping-weight']['bpa'] = False

    input_simulation_options.add_option('--cost-weight',
                                        help="Set the weight for cost term when tracking",
                                        action='store',
                                        dest='cost_weight',
                                        type='float',
                                        default=1)
    guiOverride['--cost-weight'] = {}
    guiOverride['--cost-weight']['bpa'] = False

    ################ Xian: End Add options for tracking model ##################
