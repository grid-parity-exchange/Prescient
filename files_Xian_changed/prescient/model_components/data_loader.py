...
def load_basic_data(model):
    ...
    def piecewise_init(m, g, t):
        return []

    model.CostPiecewisePoints = Set(model.ThermalGenerators, model.TimePeriods, initialize=piecewise_init, ordered=True, within=NonNegativeReals)
    model.CostPiecewiseValues = Set(model.ThermalGenerators, model.TimePeriods, initialize=piecewise_init, ordered=True, within=NonNegativeReals)
    ...
    def validate_cost_piecewise_points_and_values_rule(m, g, t):
    ...
    #        this_generator_piecewise_values.append(this_generator_piecewise_values[-1] + 1)
        ...
        if value(m.PiecewiseType) == "NoPiecewise":
            ...
            if len(m.CostPiecewisePoints[g,t]) > 0:
       ...
       # per the requirement below, there have to be at least two piecewise points if there are any.
        ...
        new_points = sorted(list(m.CostPiecewisePoints[g,t]))
        new_values = sorted(list(m.CostPiecewiseValues[g,t]))
        ...
        if list(m.CostPiecewisePoints[g,t]) != new_points:
            m.CostPiecewisePoints[g,t].clear()
            #m.CostPiecewisePoints[g].add(*new_points) # dlw and Julia July 2014 - changed to below.
            for pcwpoint in new_points:
                m.CostPiecewisePoints[g,t].add(pcwpoint)

        if list(m.CostPiecewiseValues[g,t]) != new_values:
            m.CostPiecewiseValues[g,t].clear()
            # m.CostPiecewiseValues[g].add(*new_values) # dlw and Julia July 2014 - changed to below.
            for pcwvalue in new_values:
                m.CostPiecewiseValues[g,t].add(pcwvalue)

        return True

    model.ValidateCostPiecewisePointsAndValues = BuildCheck(model.ThermalGenerators, model.TimePeriods, rule=validate_cost_piecewise_points_and_values_rule)
    ...
    def minimum_production_cost(m, g, t):
        if len(m.CostPiecewisePoints[g,t]) > 1:
            return m.CostPiecewiseValues[g,t].first() * m.FuelCost[g]
        else:
            return  m.FuelCost[g] * \
    ...
    def power_generation_piecewise_points_rule(m, g, t):
        ...
        minimum_production_cost = value(m.MinimumProductionCost[g,t]) / value(m.FuelCost[g])
        ...
        if piecewise_type == "Absolute":
           ...
           piecewise_values = list(m.CostPiecewiseValues[g,t])
           piecewise_points = list(m.CostPiecewisePoints[g,t])
           ...
           if len(m.CostPiecewisePoints[g,t]) > 0:
              PowerBreakpoints = list(m.CostPiecewisePoints[g,t])
              ...
              IncrementalCost = list(m.CostPiecewiseValues[g,t])
              CostBreakpoints = {}
              CostBreakpoints[0] = 0
